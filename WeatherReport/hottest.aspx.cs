...
{
    public partial class hottest : System.Web.UI.Page
    {
        ...
        protected void GetWeatherInfo()
        {
            try
            {
                ...
                List<root> weatherInfoList = new List<root>();
                ...
                weatherInfoList = weatherInfoList.OrderByDescending(w => w.main.temp_max).ToList();

                foreach (var weatherInfo in weatherInfoList.Take(10))
                {
                    ...
                    if (Session["username"] != null)
                    {
                        string city = weatherInfo.name.ToString();
                        weatherDiv.Controls.Add(new LiteralControl("<br>"));
                        weatherDiv.Controls.Add(new LiteralControl("<br>"));
                        weatherDiv.Controls.Add(new LiteralControl("<br>"));
                        lb = new LinkButton();
                        ...
                        lb.Click += (sender, e) => LinkButton_Click(city, sender, e);
                    }
        ...
        protected void LinkButton_Click(string city, object sender, EventArgs e)
        {
            ...
            string sql = "insert into bookmarks values(@username, @city)";
            try
            {
                conn.Open();
